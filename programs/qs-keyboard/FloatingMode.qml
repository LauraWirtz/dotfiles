...
PanelWindow {
	...
	visible: KeyboardService.visible && KeyboardService.mode == KeyboardService.Floating
	...
	KeyboardButton {
		...
		icon.name: "input-keyboard-virtual-hide"
		icon.color: "white"
		icon.width: 32
		icon.height: 32

		state: (pressed || dragHandler.active) ? "ACTIVE" : ""
		onClicked: KeyboardService.mode = KeyboardService.Docked
